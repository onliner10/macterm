...
`GhosttyKit.xcframework` and the `Macterm/Resources/{ghostty,terminfo,…}` contents are gitignored artifacts downloaded by `mise run setup` — **every fresh checkout, including a git worktree, must run `mise run setup`** before it can build. Don't symlink them from another checkout: setup normally uses a presence check rather than a version check, so a symlinked copy silently goes stale. The one capability exception is `GHOSTTY_ACTION_OUTPUT_ACTIVITY`: setup replaces a present framework that lacks this required ABI (probed by globbing the xcframework's slice dirs for `Headers/ghostty.h`, so a renamed slice can't turn the check into a silent re-download every run). The replacement is downloaded and validated in a scratch dir and only then swapped in, so a release that turns out to lack the ABI is a no-op rather than a regression that strands the tree with no framework. Note this makes the app unbuildable against any GhosttyKit predating the fork patch — relevant when bisecting across the commit that introduced it, since earlier checkouts need no such ABI and build fine against older releases. To refresh any other stale artifact, delete it and re-run setup.
